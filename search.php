...
<section class="posts">
    ...
    $sql = "SELECT * FROM post WHERE _status='1' and _body LIKE '%$searchParam%'";
    $postsData = $conn->query($sql);
    if($postsData->num_rows > 0){
        ?>
        <h2><?= $_GET['search'] ?> aramasına ait sonuçlar</h2>
        <?php
    foreach($postsData as $post){ ?>
        ...
        <?php } }else{?>
            <h2 style="margin-top: 50px;"><?= $_GET['search'] ?> aramasına ait sonuç bulunamadı.</h2>
            <?php } ?>
